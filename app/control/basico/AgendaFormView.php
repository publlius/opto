...
class AgendaFormView extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $this->fc->setEventUpdateAction(new TAction(array('AgendaForm', 'onUpdateEvent')));
        $this->fc->setCurrentView('agendaWeek');
        $this->fc->setTimeRange('07:00', '20:00');
        $this->fc->enablePopover('Agenda', " {observacao} ");
        ...
        try
        {
            ...
            if ($events)
            {
                foreach ($events as $event)
                {
                    ...
                    $event_array['title'] = TFullCalendar::renderPopover($event->render(" {titulo} "), $event->render("Agenda"), $event->render(" {observacao} "));
